...
void Field::SetIn(Coordinates in){
    this->in = {in.x, in.y};
}
void Field::SetOut(Coordinates out){
    this->out = {out.x, out.y};
}
...
void Field::SetMonsterCount(int k) {
    monstercount = monstercount + k;
}

void Field::SetTeleports(Coordinates one, Coordinates two){
    teleports.first.x = one.x;
    teleports.first.y = one.y;
    teleports.second.x = two.x;
    teleports.second.y = two.y;
}
